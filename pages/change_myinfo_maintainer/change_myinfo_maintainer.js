Page({
  ...
  onLoad: function (options) {
    var that = this
    var no_maintainer = wx.getStorageSync('no')
    this.setData({
      no_maintainer:no_maintainer
    })
    wx.request({
      url: 'https://www.bbin.design/api/myinfo2.php',
      method:"get",
      header: {
  ...
  formSubmit: function (e) {
    //首先是用var函数获取input的输入信息
        var no_maintainer = e.detail.value.no_maintainer;
        var pass_maintainer = e.detail.value.pass_maintainer;
        var phone_maintainer = e.detail.value.phone_maintainer;
    //判断一下这个人输入没，有一个没输入就用showToast提醒他下
        if (pass_maintainer == '' || phone_maintainer == '' || phone_maintainer == null || phone_maintainer == null ) {
          wx.showToast({
            title: '信息未修改',
            icon: 'error',
            duration: 1000
    ...
    //wx.request是微信小程序发送到后台的一种方式
          wx.request({
            url: "https://www.bbin.design/api/changemyinfo2.php",//写清楚你要调用哪一个后台文件
            method: 'POST',//method有两种方式，发送一般用POST，接收一般用GET
            data: {//data里面的数据你把它当成一个快递，包装所有文件然后发送出去
              no_maintainer:no_maintainer,
              pass_maintainer:pass_maintainer,
